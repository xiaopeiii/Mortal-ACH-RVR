...
def train():
    ...
    if strict_enabled:
        raise RuntimeError(
            'offline warmup does not support strict alignment; '
            'set strict.enabled=false or use train_ach_rvr_online.py for strict training'
        )
    ...
    train_brain = bool(ach_cfg.get('train_brain', True))
    train_oracle_brain = bool(rvr_cfg.get('train_oracle_brain', True))

    model = AchRvrModel(version=version, **config['resnet']).to(device)

    model.private_brain.requires_grad_(train_brain)
    model.policy_y.requires_grad_(True)
    model.value_head.requires_grad_(True)
    model.oracle_brain.requires_grad_(rvr_enabled and train_oracle_brain)
