...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate mode12 5090 FAST offline/online configs.")
    ...
    off["control"]["batch_size"] = 512
    off["control"]["opt_step_every"] = 2
    off["dataset"]["num_workers"] = 12
    off["dataset"]["file_batch_size"] = 24
    off["optim"]["max_grad_norm"] = 0.6
    off["optim"]["scheduler"]["peak"] = 1.8e-4
    off["optim"]["scheduler"]["final"] = 1.8e-5
    off["optim"]["scheduler"]["warm_up_steps"] = 6000
    off["optim"]["scheduler"]["max_steps"] = 300000
    off["control"]["test_every"] = 30000
    off["grp"]["control"]["batch_size"] = 1536

    on["control"]["batch_size"] = 384
    on["control"]["opt_step_every"] = 1
    on["control"]["submit_every"] = 300
    on["dataset"]["num_workers"] = 10
    on["dataset"]["file_batch_size"] = 20
    on["train_play"]["default"]["games"] = 800
    on["train_play"]["default"]["boltzmann_epsilon"] = 0.0
    on["train_play"]["default"]["boltzmann_temp"] = 1.0
    on["train_play"]["default"]["top_p"] = 1.0
    on["cql"]["online_q_weight"] = 0.15
    on["optim"]["max_grad_norm"] = 0.25
    on["optim"]["scheduler"]["peak"] = 1.2e-5
    on["optim"]["scheduler"]["final"] = 3e-6
    on["optim"]["scheduler"]["warm_up_steps"] = 25000
    on["optim"]["scheduler"]["max_steps"] = 300000
    on["online"]["server"]["capacity"] = 2400
    on["online"]["server"]["force_sequential"] = False

    off_out = cfg_dir / "config.mode12.5090.fast.toml"
    on_out = cfg_dir / "config.mode12.5090.fast.online.toml"
